...
import ConversationCoachPage from '@/pages/Supervision/ConversationCoachPage.vue';
...
describe('ConversationCoachPage.vue', () => {
    const localVue = createLocalVue();
    let wrapper: Wrapper<ConversationCoachPage>;

    localVue.use(BootstrapVue);
    localVue.use(Vuex);

    const setWrapper = () => {
        wrapper = shallowMount(ConversationCoachPage, {
            localVue,
            i18n,
            store: createStore(),
        });
    ...
    it('should render a translated title', () => {
        ...
        // ASSERT
        expect(wrapper.findComponent(Heading).text()).toBe('Hulpvragen gesprekscoach');
    });
});
